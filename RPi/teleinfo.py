...
# https://github.com/lhuet/teleinfo/tree/master/python

import sys, serial, time, datetime, pprint, logging, logging.config, sqlite3, mysql.connector
...
def lectureTrame(ser):
	...
	# Lecture d'eventuel caractere avant le debut de trame Jusqu'au caractere \x02 + \n (= \x0a)
	trame = list()
	while trame[-2:]!=['\x02','\n']:
...
def ligneToCSV(lignes, cles):
	...
	for cle in cles:
		# Attention, transformation de String en Int ... Toutes les cles ne fonctionnent pas !!! Double conversion pour 
		# supprimer les 0 a gauche
		valeur = str(int(lignes[cle]))
		ligneCSV.append(valeur)
	return ";".join(ligneCSV)

def ligneToSQLite(lignes):
	db = sqlite3.connect('/home/pi/teleinfo/db/compteurElec.db', detect_types = sqlite3.PARSE_COLNAMES)
	c = db.cursor()
...
if __name__ == '__main__':
	...
	logging.config.fileConfig('/home/pi/teleinfo/logging.conf')
	logger = logging.getLogger(__name__)
